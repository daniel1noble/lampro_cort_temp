...
setwd("~/Dropbox/Pilot_cort_crino")
liz_ondi <- read.csv ("Pilot_cort_mortality_db.csv") %>%  # ondi data  database data
...
fig2.svl <- ggboxplot(Liz, x = "hormone", y = "bd_svl_orig_mm",
  ...
  labs(x = "Temperature (°C)", y = "Hatchling SVL") +
  labs(color='Hormone Level')+
...
fig2.bci <- ggboxplot(Liz, x = "temp", y = "hatch_bc_residuals",
  ...
  labs(color='Body Condition Index')+
...
Fig2.mass.h <- ggboxplot(Liz, x = "hormone", y = "bd_mass_orig",
  ...
  stat_compare_means(comparisons = list(c("control","low"), c("control","high"), c("low","high"))) +
  stat_compare_means(method = "anova")+
  labs(x = "Temperature (°C)", y = "Hatchling Mass") +
  labs(color='Hormone Level')+
