...
CORT significantly decreased hatchling mass ($F_{1,125}$ = `r Mass_hatch_mod_anova[2,4]`, p = `r Mass_hatch_mod_anova[2,5]`), but only between control and the highest dose (marginalised mean: `r Mass_hormone_hatch_mod_emm_cont[1,"estimate"]` $\pm SE$ `r Mass_hormone_hatch_mod_emm_cont[1,"SE"]`, t = `r Mass_hormone_hatch_mod_emm_cont[1,"t.ratio"]`, df = `r Mass_hormone_hatch_mod_emm_cont[1,"df"]`, p = `r p_value(Mass_hormone_hatch_mod_emm_cont[1,"p.value"])`). Hatching SVL was similarly affected by CORT ($F_{1,125}$ = `r SVL_hatch_mod_anova[2,4]`, p = `r p_value(SVL_hatch_mod_anova[2,5])`) compared to control treatments, but again, only at high CORT levels (marginalised mean: `r SVL_hatch_mod_emm_cont[1,"estimate"]` $\pm SE$ `r SVL_hatch_mod_emm_cont[1,"SE"]`, t = `r SVL_hatch_mod_emm_cont[1,"t.ratio"]`, df = `r SVL_hatch_mod_emm_cont[1,"df"]`, p = `r p_value(SVL_hatch_mod_emm_cont[1,"p.value"])` @fig-Figure_1).  

High temperatures also contributed to decreased hatchling mass ($F_{1,125}$ = `r Mass_hatch_mod_anova[1,4]`, p = `r p_value(Mass_hatch_mod_anova[1,5])`; @fig-Figure_2) and resulted in poorer body condition ($F_{1,125}$ = `r condition_hatch_mod_anova[1,4]`, p = `r p_value(condition_hatch_mod_anova[1,5])`; @fig-Figure_2 B). Temperature treatment did not have an overall effect on SVL ($F_{1,125}$ = `r SVL_hatch_mod_anova[1,4]`, p = `r p_value(SVL_hatch_mod_anova[1,5])`) and hormonal treatment did not affect body condition ($F_{1,125}$ = `r condition_hatch_mod_anova[2,4]`, p = `r p_value(condition_hatch_mod_anova[2,5])`).
