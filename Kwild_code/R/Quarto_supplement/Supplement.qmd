...
rm(list=ls())
sessionInfo()
pacman::p_load(dplyr, flextable, knitr, here, dplyr, tidyverse, ggpubr, lme4, emmeans, car, lmerTest, lubridate, performance, cowplot, interp, ggpubr, ggplot2, stats, rotl, dplyr, viridis, patchwork, interp, akima, fields, MASS, magick)
...
Hormonal treatment did not affect time to hatch ($F_{`r hatch_dev_mod_anova$Df[2]`,`r hatch_dev_mod_anova$Df[3]`}$ = `r hatch_dev_mod_anova[2,4]`, p = `r p_value(hatch_dev_mod_anova[2,5])`), but as expected, developmental temperature significantly decreased development time ($F_{1,117}$ = `r hatch_dev_mod_anova[1,4]`, p = `r p_value(hatch_dev_mod_anova[1,5])`; Figure S1). On average, the incubation duration for the hot treatment was `r hatch_dev_mod_emm_value[1,2]` days (`r hatch_dev_mod_emm_value[1,3]`$\pm SE$) and`r hatch_dev_mod_emm_value[2,2]` days (`r hatch_dev_mod_emm_value[2,3]`$\pm SE$). 

High CORT significantly decreased hatchling mass ($F_{1,125}$ = `r Mass_hatch_mod_anova[2,4]`, p = `r Mass_hatch_mod_anova[2,5]`) and SVL ($F_{1,125}$ = `r SVL_hatch_mod_anova[2,4]`, p = `r p_value(SVL_hatch_mod_anova[2,5])`) in comparison to control treatments (@fig-Figure_1). Additionally high temperature treatment decreased hatchling mass ($F_{1,125}$ = `r Mass_hatch_mod_anova[1,4]`, p = `r p_value(Mass_hatch_mod_anova[1,5])`; Fig. 2A) and resulted in poor body condition ($F_{1,125}$ = `r condition_hatch_mod_anova[1,4]`, p = `r p_value(condition_hatch_mod_anova[1,5])`; Fig. 2B). Temperature treatment did not have an overall effect on SVL ($F_{1,125}$ = `r SVL_hatch_mod_anova[1,4]`, p = `r p_value(SVL_hatch_mod_anova[1,5])`) and hormonal treatment did not effect body condition (F$_{1,125}$ = `r condition_hatch_mod_anova[2,4]`, p = `r p_value(condition_hatch_mod_anova[2,5])`).
...
```{r, Table_1}
#| label: tbl-Table_1
#| tbl-cap: "Anova table for hatchling mass by developmental enviroment"
#| echo: FALSE
#| warning: FALSE
Mass_hatch_mod_anova_tbl <- Mass_hatch_mod_anova %>%
  mutate(test = "Hatchling Mass") %>% 
  rownames_to_column(var = "variable")

Table_1 <- Mass_hatch_mod_anova_tbl[, c(6, 1,2,3,4, 5)]

Tbl.1 <-flextable(Table_1) %>% 
  autofit(part = "all") %>% 
  bold(i = which(Table_1$`Pr(>F)` < 0.05), j = 2:6) %>% 
...
```{r, Table_2}
#| label: tbl-Table_2
#| tbl-cap: "Anova table for hatchling SVL by developmental enviroment"
#| echo: FALSE
#| warning: FALSE
SVL_hatch_mod_anova_tbl <- SVL_hatch_mod_anova%>%
  mutate(test = "Hatchling SVL") %>% 
  rownames_to_column(var = "variable")

Table_2 <- SVL_hatch_mod_anova_tbl[, c(6, 1,2,3,4, 5)]

Tbl.2 <-flextable(Table_2) %>% 
  autofit(part = "all") %>% 
  bold(i = which(Table_1$`Pr(>F)` < 0.05), j = 2:6) %>% 
  align(align ="center", part = "all") %>% 
  flextable::font(part = "all", fontname = "Times New Roman") %>% 
