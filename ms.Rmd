...
```{r, models, eval = TRUE, echo = TRUE, results = "hide", messages = FALSE}
...
model <- lm(egg_mass ~ trt, data = data)
summary(model)
coef <- coef(model)
```

The intercept is `r coef[1]`. 
...
Flextable is amazing. Have a good read of the documentation [here](https://ardata-fr.github.io/flextable-book/) if you need it as LOTS can be modified. We know that the vaue is `r tab_dat[1,1]`.

```{r}
# This is me playing around with GitHub
str(mtcars)
ggplot(mtcars, aes(x = gear, y = mpg, group = cyl)) +
  geom_boxplot()
```
